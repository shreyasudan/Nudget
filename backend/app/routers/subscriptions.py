...
@router.post("/detect")
async def detect_subscriptions(db: AsyncSession = Depends(get_db)):
    try:
        detected = await SubscriptionDetector.detect_recurring_charges(db)
...
async def get_subscriptions(db: AsyncSession = Depends(get_db)):
    subscriptions = await SubscriptionDetector.get_all_recurring(db)
    return subscriptions

@router.get("/gray-charges")
async def get_gray_charges(db: AsyncSession = Depends(get_db)):
