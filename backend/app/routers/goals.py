...
async def get_goals(
    ...
    goals = await GoalService.get_all_goals(db, active_only)
    return goals

@router.get("/recommendations")
async def get_goal_recommendations(db: AsyncSession = Depends(get_db)):
    recommendations = await GoalService.get_goal_recommendations(db)
...
async def update_goal_progress(
    ...
    goal = await GoalService.update_goal_progress(db, goal_id, amount)
    if not goal:
        raise HTTPException(status_code=404, detail="Goal not found")
...
async def get_goal_projection(
    ...
    projected_date = await GoalService.project_goal_completion(db, goal)
